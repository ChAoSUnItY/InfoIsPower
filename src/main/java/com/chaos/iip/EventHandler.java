...
public class EventHandler {
                ...
                switch (1) {
                    case 1:
                        (Lists.reverse((List<ItemStack>) player.getArmorInventoryList())).forEach(stack -> {
                            getAllInfos(matrix, mc, stack, armorType.next(), locator, true);
                        });
                        locator.getNextLocation(GUIElementLocator.LocatorGapTypes.ITEM);
                        player.getHeldEquipment().forEach(stack -> {
                            getAllInfos(matrix, mc, stack, handType.next(), locator, false);
                            locator.getNextLocation(GUIElementLocator.LocatorGapTypes.ITEM);
                        });
                        break;
                    case 2:
                        (Lists.reverse((List<ItemStack>) player.getArmorInventoryList())).forEach(stack -> {
                            getAllInfos(matrix, mc, stack, armorType.next(), locator, false);
                            locator.getNextLocation(GUIElementLocator.LocatorGapTypes.TEXT);
                        });
                        player.getHeldEquipment().forEach(stack -> {
                            getAllInfos(matrix, mc, stack, handType.next(), locator, true);
                        });
                        break;
                    default:
                        break;
    ...
    private static void getAllInfos(MatrixStack matrix, Minecraft mc, ItemStack stack, EquipmentSlotType slot, GUIElementLocator locator, boolean simple) {
        ...
        if (simple) {
            ...
            simpleStringDraw(matrix, font, s, 20, locator.getCurrent(), color);
            return;
        }
        int times = 0;
        if (stack.hasTag()) {
        ...
        for (ITextComponent t : texts)
            simpleStringDraw(matrix, font, t.getString(), 2, locator.getNextLocation(GUIElementLocator.LocatorGapTypes.TEXT), t.getStyle().getColor() != null ? t.getStyle().getColor().getColor() : color);
        if (times == 0)
            return;
