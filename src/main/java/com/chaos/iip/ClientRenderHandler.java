...
//                        .getNextLocation(GUIElementLocator.LocatorGapTypes.TEXT), color);
    ...
    private void getAllInfo(MatrixStack matrix, MinecraftClient mc, ItemStack stack, GUIElementLocator locator, boolean simple) {
        ...
        if (item.isFood()) {
            ...
            texts.add(DisplayContentHelper.subContents[0].copy().append(" : +" + food.getHunger()).formatted(Formatting.GOLD));
            texts.add(DisplayContentHelper.subContents[1].copy().append(" : +" + food.getSaturationModifier()).formatted(Formatting.GOLD));
        }
        ...
        if (stack.hasTag()) {
            ...
            if (stack.getEnchantments().size() > 0) {
                ...
                ListTag list = tag.getCompound("BlockEntityTag").getList("Items", 10);
                ...
                for (int i = 0, p = locator.getCurrent() + 20; i < list.size(); i++) {
                    ...
                    drawItem(font, renderer, ItemStack.fromTag((CompoundTag) list.get(i)), InfoIsPower.config.renderType == RenderType.LEFT ? 2 + (i % 9) * 20 : 165 - (2 + (i % 9) * 20), p - 3);
                }
                ...
                if (type == -1) {
                    ...
                    CompoundTag nbt = list.getCompound(target);
...
//                texts.addAll(tooltip);
        ...
        GuiUtils.renderTooltip(matrix, Lists.transform(texts, Text::asOrderedText), InfoIsPower.config.renderType == RenderType.LEFT ? -7 : mc.getWindow().getScaledWidth() + 11, locator.getNextLocation(GUIElementLocator.LocatorGapTypes.ITEM) + 3);
